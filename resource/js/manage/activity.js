...
layui.use(['layer', 'element', 'form', 'upload', 'table'], function(){
	...
	// 添加活动
	form.on('submit(activitylist_add)', function(data){
		$.ajax({
			url: "/jdwlkj/activity/add",
			data: data.field,
			type: "POST",
			dataType: "json",
			timeout: 15000,
	...
	// 活动开关
    form.on('switch(activityState)', function(obj){
        var val = obj.elem.checked ? 1 : 2;
        $.ajax({
            url: "/jdwlkj/activity/activityOnoff",
            data: {
                id: this.value,
                field: "state",
                val: val
		...
		switch (obj.event) {
			case 'edit':
				layer.open({
					...
					area: ['80%', '80%'],
					title: '编辑活动',
					content: '/jdwlkj/activity/edit?id='+obj.data.id
				});
				break;
					...
					$.ajax({
						...
						success: function(msg){
							...
							layer.msg(alertStr, {time: 2000}, function(){
								if(msg==1){
									layer.closeAll();
									layui.table.reload('activityList');
								}
							});
						},
						complete: function(){
