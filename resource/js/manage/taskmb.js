...
layui.use(['layer', 'element', 'form', 'upload', 'table', 'laydate'], function(){
     ...
     */
    // 头部左侧工具栏事件
    table.on('toolbar(taskmbList)', function(obj){
        switch(obj.event){
            case 'add':
                layer.open({
					type: 2,
					title: "添加任务模板",
					area: ['95%','95%'],
					content: "/jdwlkj/bet/taskmbAdd"
				});
                break;

            case 'del':
                var data = table.checkStatus('taskmbList').data;
                if (data.length == 0) {
                    layer.msg('请选择要删除的任务模板');
                    return false;
                }
                layer.confirm('确认操作？', {
                    btn: ['确定','取消'] //按钮
    ...
    // 监听工具条
    table.on('tool(taskmbList)', function(obj){
        switch (obj.event) {
            ...
            case 'edit':
                layer.open({
					...
					area: ['95%', '95%'],
					title: '编辑任务模板',
					content: '/jdwlkj/bet/taskmbEdit?id='+obj.data.id
				});
                break;
            ...
            success: function(msg){
                ...
                layer.msg(alertStr, {time: 2000}, function(){
                    if(msg==1){
						...
						parent.layui.table.reload('taskmbList');
                    }
                });
            },
            complete: function(){
